...
function datosModal(date) {
  ...
  fetch(`http://127.0.0.1:5000/calendario/${dateremplazo}`, requestOptions)
  .then(response => {
    validarSesion(response);
    return response.json()})
  .then(data => {
    ...
    if (data.length > 0) {
      data.forEach(event => {
        ...
        listItem.innerText = `Cliente: ${event.idUsuario},Departamento Nro: ${event.title}, Turno: ${event.idCategoria}`; 
...
function loadEvents() {
  ...
  fetch(`http://127.0.0.1:5000/calendario`, requestOptions)
    .then(response => {
      validarSesion(response);
      return response.json()})
    .then(data => {
...
function saveEvent() {
  ...
  if (eventTitleInput.value) {
    ...
    fetch(`http://127.0.0.1:5000/calendario`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-access-token': token,
...
function cancelarEvent(){
  ...
  const confirmacion = confirm('¿Está seguro que desea cancelar su reserva altual?');
  ...
  if (confirmacion) {
    ...
    fetch(`http://127.0.0.1:5000/calendario/${idUsuario}`, {
      method: 'DELETE',
      headers: {
       'Content-Type': 'application/json',
        'x-access-token': token,
...
function cerrarSesion(mensaje){
  ...
  window.location.href = "https://edificio325.com.ar/";
  return;
}
