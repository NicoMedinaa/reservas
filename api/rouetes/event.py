...
def nuevoEvento():
    ...
    if row is not None: #En caso de que dos usuarios intenten reservar un turno al mismo tiempo
        return jsonify({'message': "A ocurrido un error inesperado, pruebe a reiniciar la pagina"}), 409
...
def detallesDia(date):
    ...
    for row in datos:
        ...
        cur.execute('SELECT departamento FROM cliente WHERE id = %s', (row[1],))
        obj.title = cur.fetchone()
